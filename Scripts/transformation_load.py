...
'''
fac_map = pd.read_csv(r"/bigdata/data/target/facility_mapping.csv", index_col = 0)
...
def reference_update(og_df, new_df, id_col, df_class):
    ...
    if(og_df.empty):
        ...
        if len(deact_id) > 0:
            deactivate_db(df_class, 'current_ind', deact_id)
...
def send_email(sender='bigdata@ghsc-psm.org',to='wirshad@ghsc-psm.org', subject='Transformation and Load Process Completed', body= 'Transformation and Load process completed. Please review the attched log files for any exceptions.', attach= log_path/error_log_file, attach1=log_path/process_log_file):
...
if __name__ == '__main__':
    ...
    try:
        insert_db(facility_status, facility_status_new)
        fac_stat_length = len(facility_status_new.index)
        print("Facility Status Table Updated: ")
        print("Completed on:", str(datetime.now()))
        print('Rows inserted: ', str(fac_stat_length))
        print('\n')
    except:
        e = sys.exc_info()
    ...
    try:
        insert_db(stock_detail5, stock_df)
